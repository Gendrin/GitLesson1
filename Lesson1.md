...
#
Выполняем коммит и переходим к первой, добавляем запись, затем ко второй добавляем запись. Производим первое слияние (первая ветка)

Добавляем записи в первую ветку, без конфликта. И производим е коммит.
Прозводим ручную корриктеровку, убирая наложение и производим commit.
Добавляем записи во вторую ветку, без конфликта. И производим ее коммит.
#
Добавляем сохранения экранов по первым двум действиям.
![CMD](image/17.png "Переход на первую ветку и ее редактирование")
#
![CMD](image/18.png "Переход на первую ветку и ее редактирование")
#
![CMD](image/19.png "Переход на первую ветку и ее редактирование")
#
Произошел конфликт, вручную производим корректировку и последующий коммит в main.

![CMD](image/20.png "Переход на вторую ветку и ее редактирование")
#
![CMD](image/21.png "Переход на третью ветку и поглощение 4й без конфликта")

В данной ситуации всегда будут происходить конфликты т.к.  происходит добавление к ветке Main каждой последующей ветки, а все остальные четыре ветки стартовали от предыдущего ее состояния.
#
Поэтому встаем на четвертую ветку (добавляем изменения) и забираем их третьей веткой. По логике измения должны пройти без конфликта.
Предположение подтвердилось, снимки экрана ниже:
#
![CMD](image/22.png "Переход на четвертую ветку и ее редактирование")
#
![CMD](image/23.png "Переход на четвертую ветку и ее редактирование")
#
![CMD](image/24.png "Переход на третью ветку и поглощение 4й без конфликта")
